...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if (ns_aa == 1)
  {
    ...
    double X = (origin.x + 0.5) / sampleBuffer.w*1.0;
    double Y = (origin.y + 0.5) / sampleBuffer.h*1.0;
    ...
    for(int i = 0; i < ns_aa; i++) {
      ...
      Vector2D rndSample = gridSampler->get_sample();

      double X = (origin.x + rndSample.x) / sampleBuffer.w*1.0;
      double Y = (origin.y + rndSample.y) / sampleBuffer.h*1.0;
