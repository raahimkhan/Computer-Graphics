...
Ray Camera::generate_ray(double x, double y) const { 
  ...
  Vector3D r_d = Vector3D(bottom_left.x + (top_right.x - bottom_left.x) * x, bottom_left.y + (top_right.y - bottom_left.y) * y, -1.0); 
  r_d = c2w * r_d;
  r_d.normalize();
  
  Ray ray = Ray(Vector3D(pos), Vector3D(r_d), fClip, 0);
  ray.min_t = nClip;
  return ray;
}
