...
//Part7: Rasterize a triangle.
void DrawRend::rasterize_triangle( float x0, float y0,
                        float x1, float y1,
                        float x2, float y2,
                        Color color, Triangle *tri) {

//Part7: Filling in sub pixels

  float minX = min(min(x0, x1), x2) ;
  float maxX = max(max(x0, x1), x2) ;
  float minY = min(min(y0, y1), y2) ;
  float maxY = max(max(y0, y1), y2) ;
  float l = sqrt(sample_rate) ;

  for (int i = minX ; i < round(maxX) ; i++) 
  {
    for (int j = minY ; j < round(maxY) ; j++)
    {
      for (int row = 0 ; row < round(l) ; row++)
      {
        for (int col = 0 ; col < round(l) ; col++)
        {
          float xNew = i + (row + 0.5) / l ;
          float yNew = j + (col + 0.5) / l ;
          bool check1 = dot_product(x0, y0, x1, y1, xNew, yNew) >= 0 ;
          bool check2 = dot_product(x1, y1, x2, y2, xNew, yNew) >= 0 ;
          bool check3 = dot_product(x2, y2, x0, y0, xNew, yNew) >= 0 ;
          bool check4 = dot_product(x0, y0, x2, y2, xNew, yNew) >= 0 ;
          bool check5 = dot_product(x1, y1, x0, y0, xNew, yNew) >= 0 ;
          bool check6 = dot_product(x2, y2, x1, y1, xNew, yNew) >= 0 ;
          if (check1==true && check2==true && check3==true || check4==true && check5==true && check6==true)
          {
            samplebuffer[j][i].fill_color(row, col, color) ;
          }
        }
      }
    }
