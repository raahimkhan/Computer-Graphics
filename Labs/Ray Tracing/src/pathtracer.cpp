...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  Spectrum output ;
  Vector2D orig = Vector2D(x, y) ;
  if (ns_aa == 1) {
    double X = (orig.x + 0.5) / sampleBuffer.w * 1.0 ;
    double Y = (orig.y + 0.5) / sampleBuffer.h * 1.0 ;
    Ray pixel_ray = camera -> generate_ray(X, Y) ;
    pixel_ray.depth = max_ray_depth ;
    return trace_ray(pixel_ray, true) ;
  }

  // Part 2

  for (int i = 0 ; i < ns_aa ; i++) {
    Vector2D add = gridSampler -> get_sample() ;
    double X = (orig.x + add.x) / sampleBuffer.w * 1.0 ;
    double Y = (orig.y + add.y) / sampleBuffer.h * 1.0 ;
    Ray pixel_ray = camera -> generate_ray(X, Y) ;
    pixel_ray.depth = max_ray_depth ;
    output += trace_ray(pixel_ray, true) ;
  }
